...
public class SqlDAOImpl implements SqlDAO {
    ...
    public void addStudent(Student bean) {
        ...
        String sql =
                "insert into user(password,user_name,phone_number,access_level,student_id,address,e_mail) values(? ,? ,? ,? ,? ,? ,? )";
    ...
    public Student getStudent(int id) {
        String sql =
                "select user_id,user_name,password,phone_number,access_level,student_id,address,e_mail from user where user_id = ?";
        try (Connection c = DBUtil.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setLong(1, id);
            ps.execute();
    ...
    public Student getStudent(int id, String password) {
        String sql =
                "select user_id,user_name,password,phone_number,access_level,student_id,address,e_mail from user where user_id = ? and password = ?";
        try (Connection c = DBUtil.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setLong(1, id);
    ...
    public List<Student> searchStudent(String condition) {
        String sql =
                "select user_id,user_name,password,phone_number,access_level,student_id,address,e_mail from user ORDER BY user_id";
    ...
    public void addReservation(Reservation reservation) {
        ...
        String sql =
                "insert into reservation(id,state,user_id,application_type,application_time,required_time,place,repair_activity_id,equipment_id,repair_type,detail,remark,feedback) values(? ,? ,? ,? ,? ,? ,? ,?,?,?,?,?,?)";
        try (Connection c = DBUtil.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
    ...
    public void updateReservation(Reservation reservation) {
        // TODO Auto-generated method stub
        String sql =
                "update reservation set userid=?,state=?,application_type=?,application_time=?,required_time=?,place=?,repair_activity_id=?,equipment_id=?,repair_type=?,detail=?,remark=?,feedback=? where id=?";
        try (Connection c = DBUtil.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            // ps.setString(1, bean.getName());
            ps.execute();
    ...
    public Reservation getReservation(int id) {
        // TODO Auto-generated method stub
        String sql =
                "select state,user_id,application_type,application_time,required_time,place,repair_activity_id,equipment_id,repair_type,detail,remark,feedback from reservation where id = ?";
        try (Connection c = DBUtil.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setLong(1, id);
            ps.execute();
            ResultSet rs = ps.getResultSet();
    ...
    public List<Reservation> searchReservation(String condition) {
        // TODO Auto-generated method stub
        String sql =
                "select id,state,user_id,application_type,application_time,required_time,place,repair_activity_id,equipment_id,repair_type,detail,remark,feedback from reservation ORDER BY id";
    ...
    /*
     * isnull语句是判断该id是否存有属性值，如果存在单个属性那插入另一个属性用update语句 如果该id不存在则用insert插入一个新行
     * 
     * @param type 维修类型，1表示硬件，2表示软件 (non-Javadoc)
     * 
    ...
    public void addRepairType(int id, int type, String content) {
        // TODO Auto-generated method stub
        String isnull =
                "select id,software,hardware from repair_type where id=? and (hardware is null or software is null)";
    ...
    /*
     * isnull语句是判断这个id中是否只有一个属性，如果只有硬件或者软件的单个属性就把id整行删掉 如果硬件或软件都有就把要“删除”的属性变为null值 (non-Javadoc)
     * 
    ...
    public void deleteRepairType(int id, int type) {
        ...
        /*
         * String sql = "delete from repair_type where id = ?"; try (Connection c = DBUtil.getConnection();
         * PreparedStatement ps = c.prepareStatement(sql)) { ps.setLong(1, id); boolean num=ps.execute(); if
         * (num) { System.out.print("删除成功"); } else { System.out.print("删除失败"); } } catch (SQLException e) {
         * e.printStackTrace(); } String sql=""; if(type==1) {
         * sql="update repair_type set hardware=null where hardware =?"; }else {
         * sql="update repair_type set software=null where software =?"; } try (Connection c =
         * DBUtil.getConnection();PreparedStatement ps=c.prepareStatement(sql) ){ ps.setString(1, content);
    ...
    public void addManger(Manger manger) {
        ...
        String sql =
                "insert into manger(password,name,phone_number,access_level,address,email) values(? ,? ,? ,? ,? ,? )";
        try (Connection c = DBUtil.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
    ...
    public Manger getManger(int id, String password) {
        // TODO Auto-generated method stub
        String sql =
                "select id,name,password,phone_number,access_level,email from manger where id = ? and password = ?";
        try (Connection c = DBUtil.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, id);
            ps.setString(2, password);
            ps.execute();
